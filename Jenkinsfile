  ...
  stage('Deploy on Dev') {
  	node('master'){
    	withEnv(["KUBECONFIG=${JENKINS_HOME}/.kube/dev-config","IMAGE=${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com/${ECR_REPO_NAME}:${IMAGETAG}"]){
        	...
        	sleep 180
        	DESIRED= sh (
          		script: "~/aws1/kubectl get deployment/$DEPLOYMENT | awk '{print \$2}' | grep -v DESIRED",
          		returnStdout: true
         	).trim()
        	CURRENT= sh (
          		script: "~/aws1/kubectl get deployment/$DEPLOYMENT | awk '{print \$3}' | grep -v CURRENT",
          		returnStdout: true
         	).trim()
  ...
  node('master'){
     if (userInput['PROD_BLUE_DEPLOYMENT'] == false) {
    	withEnv(["KUBECONFIG=${JENKINS_HOME}/.kube/prod-config"]){
        	...
        	GREEN_LB = sh (
          		script: "~/aws1/kubectl get svc ${GREEN_SVC_NAME} -o jsonpath=\"{.status.loadBalancer.ingress[*].hostname}\"",
          		returnStdout: true
        	).trim()
...
stage('Patch Prod Blue Service') {
    node('master'){
      if (userInput['PROD_BLUE_DEPLOYMENT'] == false) {
      	withEnv(["KUBECONFIG=${JENKINS_HOME}/.kube/prod-config"]){
        	BLUE_VERSION = sh (
            	script: "~/aws1/kubectl get svc/${PROD_BLUE_SERVICE} -o yaml | yq .spec.selector.version",
          	returnStdout: true
        	).trim()
        	CMD = "~/aws1/kubectl get deployment -l version=${BLUE_VERSION} | awk '{if(NR>1)print \$1}'"
        	BLUE_DEPLOYMENT_NAME = sh (
            	script: "${CMD}",
          		returnStdout: true
        	).trim()
        	echo "${BLUE_DEPLOYMENT_NAME}"
          	sh """~/aws1/kubectl patch svc  "${PROD_BLUE_SERVICE}" -p '{\"spec\":{\"selector\":{\"app\":\"taxicab\",\"version\":\"${BUILD_NUMBER}\"}}}'"""
          	echo "Deleting Blue Environment..."
          	sh "~/aws1/kubectl delete svc ${GREEN_SVC_NAME}"
          	sh "~/aws1/kubectl delete deployment ${BLUE_DEPLOYMENT_NAME}"
      	}
      }
    }
}
